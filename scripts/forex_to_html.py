...
def analyze_forex_to_html(macro_tickers=None, macro_labels=None, macro_categorias=None):
    """Genera el HTML del dashboard FX.

    Args:
        macro_tickers: Dict opcional {nombre_limpio: ticker_yf} con activos
            macro (p.ej. {"GOLD": "GC=F"}). Se añaden a la descarga y entran
            al análisis como columnas, de modo que el selector y las gráficas
            los tratan igual que a un par FX. None = solo los 31 pares.
        macro_labels: Dict opcional {nombre_limpio: texto} con la etiqueta
            mostrada en el selector para cada activo macro (p.ej.
            {"GOLD": "Oro (GC=F)"}). None = nombre limpio.
        macro_categorias: Dict opcional {nombre_limpio: categoria} (p.ej.
            {"GOLD": "metales"}) para el filtro por categoría del selector.
            Las columnas que no estén aquí (los pares FX nativos) se tratan
            como categoría "fx". None = sin filtro (todas "fx").
    ...
    symbols = [
        ...
        "EURGBP=X", "EURJPY=X", "GBPJPY=X", "EURCHF=X", "AUDJPY=X", "EURAUD=X", "GBPCHF=X", "AUDNZD=X"
    ]

    if macro_tickers:
        symbols = symbols + list(macro_tickers.values())
    ...
    # Los activos macro llegan con su ticker yfinance (p.ej. "GC=F"); se
    # renombran a su nombre limpio (p.ej. "GOLD") para el selector y los JSONs.
    if macro_tickers:
        reverse_macro = {ticker: nombre for nombre, ticker in macro_tickers.items()}
        new_cols = [reverse_macro.get(n, n) for n in new_cols]
    ...
    fx_clean = {s.replace('=X', '') for s in symbols if s.endswith('=X')}

    def pips_multiplier(col: str) -> float:
        """Factor de pips: 100 en pares JPY, 10000 en el resto de FX, 1 en macro."""
        if col not in fx_clean:
            return 1.0
        return 100 if "JPY" in col else 10000
    ...
    # proporcional al nº de activos (~0.20 in/celda, moderado para no desbordar
    # la fila) y etiquetas pequeñas rotadas.
    n_corr = corr_matrix.shape[0]
    plt.figure(figsize=(14, max(8, n_corr * 0.20)))
    ...
    for col in pips_df.columns:
        multiplier = pips_multiplier(col)
    ...
    # Alto proporcional al nº de activos: con 31 quedaba a (10,8); con ~50 cada
    # barra era diminuta. ~0.32 in/barra mantiene el rango legible al crecer.
    plt.figure(figsize=(12, max(8, len(avg_range) * 0.20)))
    ...
    # Mismo criterio que Rango Diario: alto proporcional al nº de activos.
    plt.figure(figsize=(12, max(8, len(avg_m_range) * 0.20)))
    ...
    # El ranking son 20 pares fijos (head(20)[::2] × pos/neg), etiquetas largas
    # ("EURUSD / GOLD"): ancho 12, ~0.45 in/etiqueta — holgado sin desbordar la fila.
    plt.figure(figsize=(12, max(8, len(ranking_labels) * 0.22)))
    ...
    # Etiquetas legibles para el selector (los pares FX muestran su nombre).
    display_names_json = json.dumps(macro_labels or {})

    # Categoría de cada columna para el filtro del selector (los macro por su
    # categoría; los pares FX nativos no están en macro_categorias -> "fx").
    if macro_categorias:
        pair_cat_json = json.dumps(
            {col: macro_categorias.get(col, "fx") for col in data_close.columns}
        )
    else:
        pair_cat_json = "{}"
    ...
    # El value de cada padre se calcula como SUMA de sus hijos: con
    # branchvalues="total" Plotly reparte el ángulo del padre entre los
    ...
    for col in data_close.columns:
        ...
        values_map[f"inst-{col}"] = 1  # Tamaño fijo para cada par
        values_map[f"cur-{bc}"] += 1
        values_map["Market"] += 1
        colors.append(last_ret)
        hovers.append(f"{col}: {last_ret:.3f}%")

    values = [values_map[i] for i in ids]
    ...
    labels_sankey = base_list + pair_list  # legibles (pueden repetirse; Plotly usa el índice)
    ...
    <body>
        <div class="container">
            <header>
                <h1 style="color:#58a6ff; margin:0;">Quant Dashboard</h1>
                <p style="color:#8b949e; margin:5px 0 0 0;">Análisis Quant • Radar Técnico Multi-TF</p>
            </header>
            ...
            <div class="controls">
                ...
                <select id="categoryFilter" onchange="filterByCategory()"></select>
            </div>
            ...
            <div class="row-4-cols">
                <div class="card" onclick="expandImg(this.querySelector('img'), 'Volatilidad / Día Mes')">
                    <span class="zoom-icon">🔍</span>
                    <h2>Volatilidad / Día Mes</h2>
                    <img src="data:image/png;base64,{img_dom}">
                </div>
                <div class="card" onclick="expandImg(this.querySelector('img'), 'Rango Diario (%)')">
                    <span class="zoom-icon">🔍</span>
                    <h2>Rango Diario (%)</h2>
                    <img src="data:image/png;base64,{img_range}">
                </div>
                <div class="card" onclick="expandImg(this.querySelector('img'), 'Rango Mensual (%)')">
                    <span class="zoom-icon">🔍</span>
                    <h2>Rango Mensual (%)</h2>
                    <img src="data:image/png;base64,{img_m_range}">
                </div>
        ...
        <script>
            ...
            // Amplía una imagen matplotlib (Ranking, Matriz, Rangos...) en el modal,
            // igual que expandChart hace con los canvas/plotly.
            function expandImg(imgEl, title) {{
                lastExpandedId = 'img';
                const modal = document.getElementById('chartModal');
                const modalTitle = document.getElementById('modalTitle');
                const modalCanvas = document.getElementById('modalChart');
                ...
                if (modalChart) {{ modalChart.destroy(); modalChart = null; }}
                modalCanvas.style.display = 'none';
                try {{ Plotly.purge('modalPlotly'); }} catch(e) {{}}
                modalPlotly.style.display = 'block';
            ...
            pairs.forEach(p => {{
                let o = document.createElement('option'); o.value = p; o.innerHTML = displayNames[p] || p; selector.appendChild(o);
            }});
            ...
            function filterByCategory() {{
                ...
                pairs.forEach(function (p) {{
                    var show = !c || pairCat[p] === c;
                    var opt = selector.querySelector('option[value="' + p + '"]');
                    if (opt) opt.style.display = show ? '' : 'none';
                    if (show && !firstVisible) firstVisible = p;
                }});
                // Si el activo elegido ya no está visible, saltar al primero.
                if (c && pairCat[selector.value] !== c && firstVisible) {{
                    selector.value = firstVisible;
                    updateCharts();
                }}
            }}
            ...
            // Redimensiona todos los gráficos de la sección Gráficas. Se llama al
            // mostrar la pestaña: initAll() los crea con la sección en display:none
            // (la pestaña activa al cargar es FX), donde chart.js/Plotly no pueden
            // medir el contenedor y quedan con tamaño desproporcionado.
            window._ajustarGraficasFX = function() {{
                var chs = [chartPrice, chartDom, chartDow, chartPips, chartPipsDow, chartMarkov, chartKernelHist, chartSeasonality];
